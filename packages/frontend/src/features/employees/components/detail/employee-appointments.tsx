...
import type { EmployeeResponseDto } from "@/features/employees";

type EmployeeAppointmentsProps = {
...
export function EmployeeAppointments({ employee }: EmployeeAppointmentsProps) {
  ...
  // Fetch employee availability and leave days for calendar view
  ...
  const columns = createAppointmentColumns(
    ...
    handleCheckIn,
    handleCancel,
    false // showEmployee: показываем пациента (не врача)
  );
